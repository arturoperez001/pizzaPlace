...
helpers.mailGun = function(email,callback){
    ...
    let payload = {
    'from':'excited user <peterpetersonfromsales@sandboxcefcc3c653d0496da4fca075b97c3bf2.mailgun.org>',
    'to':email,
    'subject':'Your order',
    ...
    let parsedUrl = proxy+'https://api.mailgun.net/v3/sandboxcefcc3c653d0496da4fca075b97c3bf2.mailgun.org/messages';
...
helpers.payStripe = function(email,payData,callback){
    ...
    //let email=typeof(email) == 'string' && email.trim().length == 10 ? email.trim() : false;
        ...
        let amount = payData.items.reduce((accu,cv)=>{   
                                                    return (accu.quantity*accu.price)+(cv.quantity*cv.price)
                                                });
        console.log(payData);
        console.log(amount);
        let payload = {
